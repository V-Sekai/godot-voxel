...
VoxelDataBlock *VoxelDataMap::get_or_create_block_at_voxel_pos(VoxelVector3i pos) {
	VoxelVector3i bpos = voxel_to_block(pos);
...
void VoxelDataMap::set_voxel_f(real_t value, VoxelVector3i pos, unsigned int c) {
	VoxelDataBlock *block = get_or_create_block_at_voxel_pos(pos);
	VoxelVector3i lpos = to_local(pos);
	RWLockWrite lock(block->voxels->get_lock());
...
void VoxelDataMap::remove_block_internal(VoxelVector3i bpos, unsigned int index) {
...
VoxelDataBlock *VoxelDataMap::set_block_buffer(VoxelVector3i bpos, Ref<VoxelBuffer> buffer) {
...
void VoxelDataMap::copy(VoxelVector3i min_pos, VoxelBuffer &dst_buffer, unsigned int channels_mask) {
	const VoxelVector3i max_pos = min_pos + dst_buffer.get_size();

	const VoxelVector3i min_block_pos = voxel_to_block(min_pos);
	const VoxelVector3i max_block_pos = voxel_to_block(max_pos - VoxelVector3i(1, 1, 1)) + VoxelVector3i(1, 1, 1);
...
void VoxelDataMap::paste(VoxelVector3i min_pos, VoxelBuffer &src_buffer, unsigned int channels_mask, uint64_t mask_value,
				...
				for (unsigned int channel = 0; channel < VoxelBuffer::MAX_CHANNELS; ++channel) {
					...
					if (mask_value != std::numeric_limits<uint64_t>::max()) {
						...
						dst_buffer.read_write_action(dst_box, channel,
								[&src_buffer, mask_value, src_offset, channel](const VoxelVector3i pos, uint64_t dst_v) {
...
bool VoxelDataMap::is_area_fully_loaded(const Box3i voxels_box) const {
	...
	return block_box.all_cells_match([this](VoxelVector3i pos) {
