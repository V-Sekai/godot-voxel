...
void VoxelBuffer::create(VoxelVector3i size) { create(size.x, size.y, size.z); }
...
void VoxelBuffer::set_block_metadata(Variant meta) { _block_metadata = meta; }
...
void VoxelBuffer::for_each_voxel_metadata_in_area(Callable callback,
                                                  Box3i box) const {
  ERR_FAIL_COND(callback.is_null());
  for_each_voxel_metadata_in_area(box, [&callback](VoxelVector3i pos,
                                                   Variant meta) {
...
void VoxelBuffer::clear_voxel_metadata() { _voxel_metadata.clear(); }
