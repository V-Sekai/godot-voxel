...
void DirectMultiMeshInstance::set_material_override(Ref<BaseMaterial3D> material) {
	ERR_FAIL_COND(!_multimesh_instance.is_valid());
	RenderingServer &vs = *RenderingServer::get_singleton();
...
PackedFloat32Array DirectMultiMeshInstance::make_transform_3d_bulk_array(Span<const Transform3D> transforms) {
	VOXEL_PROFILE_SCOPE();
