...
struct VoxelVector3i {
  ...
  _FORCE_INLINE_ VoxelVector3i() : x(0), y(0), z(0) {}

  explicit _FORCE_INLINE_ VoxelVector3i(int xyz) : x(xyz), y(xyz), z(xyz) {}

  _FORCE_INLINE_ VoxelVector3i(int px, int py, int pz) : x(px), y(py), z(pz) {}
...
// For STL
namespace std {
template <> struct hash<VoxelVector3i> {
