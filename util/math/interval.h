...
struct Interval {
  ...
  inline Interval operator*(const Interval &other) const {
    // Note, if the two operands have the same source (i.e you are doing x^2),
    // this may lead to suboptimal results. You may then prefer using a more
    // dedicated function.
...
inline Interval operator*(float b, const Interval &a) { return a * b; }

// Functions declared outside, so using intervals or numbers can be the same
// code (templatable)
...
inline Interval clamp(const Interval &i, const Interval &p_min,
                      const Interval &p_max) {
  if (p_min.is_single_value() && p_max.is_single_value()) {
    return {::clamp(i.min, p_min.min, p_max.min),
...
inline Interval lerp(const Interval &a, const Interval &b, const Interval &t) {
  if (t.is_single_value()) {
    return Interval(Math::lerp(a.min, b.min, t.min),
                    Math::lerp(a.max, b.max, t.min));
  }
  ...
  return Interval(min(v0, v1, v2, v3, v4, v5, v6, v7),
...
inline Interval atan2(const Interval &y, const Interval &x,
                      OptionalInterval *secondary_output) {
  ...
  if (in_q1 && in_q2) {
    if (secondary_output == nullptr) {
      ...
      // We would be forced to split the interval in two, but we have to return
      // only one. For correctness, we have to return the full range...
      return Interval{-Math_PI, Math_PI};
    } else {
...
inline Interval stepify(const Interval &p_value, const Interval &p_step) {
  // TODO Division by zero returns 0, which is different from Godot's stepify.
...
// Prefer this instead of doing polynomials with a single interval, this will
// provide a more optimal result
inline Interval polynomial_second_degree(const Interval x, float a, float b,
                                         float c) {
  // a*x*x + b*x + c
