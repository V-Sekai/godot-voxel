...
public:
  ...
  Box3i(VoxelVector3i p_pos, VoxelVector3i p_size) : pos(p_pos), size(p_size) {}

  Box3i(int ox, int oy, int oz, int sx, int sy, int sz)
      : pos(ox, oy, oz), size(sx, sy, sz) {}
  ...
  // max is exclusive
  static inline Box3i from_min_max(VoxelVector3i p_min, VoxelVector3i p_max) {
  ...
  static inline Box3i get_bounding_box(Box3i a, Box3i b) {
    ...
    VoxelVector3i max_a = a.pos + a.size;
    VoxelVector3i max_b = b.pos + b.size;
  ...
  bool inline contains(VoxelVector3i p_pos) const {
    const VoxelVector3i end = pos + size;
    return p_pos.x >= pos.x && p_pos.y >= pos.y && p_pos.z >= pos.z &&
           p_pos.x < end.x && p_pos.y < end.y && p_pos.z < end.z;
  }

  bool inline contains(const Box3i other) const {
    const VoxelVector3i other_end = other.pos + other.size;
    const VoxelVector3i end = pos + size;
    return other.pos.x >= pos.x && other.pos.y >= pos.y &&
           other.pos.z >= pos.z && other_end.x <= end.x &&
           other_end.y <= end.y && other_end.z <= end.z;
  }

  String to_string() const {
    return String("(o:{0}, s:{1})")
        .format(varray(pos.to_vec3(), size.to_vec3()));
  }
  ...
  struct NoAction {
    inline void operator()(const VoxelVector3i pos) {}
  };

  template <typename A> inline void for_each_cell(A a) const {
    VoxelVector3i max = pos + size;
    VoxelVector3i p;
  ...
  // Returns true if all cells of the box comply with the given predicate on
  // their position.
  ...
  template <typename A> void difference(const Box3i &b, A action) const {
    ...
    VoxelVector3i a_min = a.pos;
    VoxelVector3i b_min = b.pos;
    VoxelVector3i a_max = a.pos + a.size;
    VoxelVector3i b_max = b.pos + b.size;

    if (a_min.x < b_min.x) {
      VoxelVector3i a_rect_size(b_min.x - a_min.x, a.size.y, a.size.z);
    ...
    if (a_min.y < b_min.y) {
      VoxelVector3i a_rect_size(a.size.x, b_min.y - a_min.y, a.size.z);
      action(Box3i(a_min, a_rect_size));
      a_min.y = b_min.y;
      a.pos.y = b.pos.y;
    ...
    if (a_min.z < b_min.z) {
      VoxelVector3i a_rect_size(a.size.x, a.size.y, b_min.z - a_min.z);
      action(Box3i(a_min, a_rect_size));
      a_min.z = b_min.z;
      a.pos.z = b.pos.z;
    ...
    if (a_max.x > b_max.x) {
      VoxelVector3i a_rect_pos(b_max.x, a_min.y, a_min.z);
      VoxelVector3i a_rect_size(a_max.x - b_max.x, a.size.y, a.size.z);
      action(Box3i(a_rect_pos, a_rect_size));
      a_max.x = b_max.x;
      a.size.x = a_max.x - a_min.x;
    }
    if (a_max.y > b_max.y) {
      VoxelVector3i a_rect_pos(a_min.x, b_max.y, a_min.z);
      VoxelVector3i a_rect_size(a.size.x, a_max.y - b_max.y, a.size.z);
      action(Box3i(a_rect_pos, a_rect_size));
      a_max.y = b_max.y;
      a.size.y = a_max.y - a_min.y;
    }
    if (a_max.z > b_max.z) {
      VoxelVector3i a_rect_pos(a_min.x, a_min.y, b_max.z);
      VoxelVector3i a_rect_size(a.size.x, a.size.y, a_max.z - b_max.z);
      action(Box3i(a_rect_pos, a_rect_size));
    }
  }
  ...
  template <typename F> void for_inner_outline(F f) const {
    ...
    VoxelVector3i min_pos = pos;
    VoxelVector3i max_pos = pos + size;

    // Top and bottom
    for (int z = min_pos.z; z < max_pos.z; ++z) {
      for (int x = min_pos.x; x < max_pos.x; ++x) {
        f(VoxelVector3i(x, min_pos.y, z));
        f(VoxelVector3i(x, max_pos.y - 1, z));
      }
    }
    ...
    for (int x = min_pos.x; x < max_pos.x; ++x) {
      for (int y = min_pos.y; y < max_pos.y; ++y) {
        f(VoxelVector3i(x, y, min_pos.z));
        f(VoxelVector3i(x, y, max_pos.z - 1));
      }
    }
    ...
    for (int z = min_pos.z; z < max_pos.z; ++z) {
      for (int y = min_pos.y; y < max_pos.y; ++y) {
        f(VoxelVector3i(min_pos.x, y, z));
        f(VoxelVector3i(max_pos.x - 1, y, z));
      }
    }
  }

  inline Box3i padded(int m) const {
    return Box3i(pos.x - m, pos.y - m, pos.z - m, size.x + 2 * m,
                 size.y + 2 * m, size.z + 2 * m);
  }

  // Converts the rectangle into a coordinate system of higher step size,
  // rounding outwards of the area covered by the original rectangle if divided
  // coordinates have remainders.
  inline Box3i downscaled(int step_size) const {
    Box3i o;
    o.pos = pos.floordiv(step_size);
    // TODO Is that ceildiv?
    VoxelVector3i max_pos = (pos + size - VoxelVector3i(1)).floordiv(step_size);
    o.size = max_pos - o.pos + VoxelVector3i(1);
    return o;
  }

  // Converts the rectangle into a coordinate system of higher step size,
  // rounding inwards of the area covered by the original rectangle if divided
  // coordinates have remainders. This is such that the result is included in
  // the original rectangle (assuming a common coordinate system). The result
  // can be an empty rectangle.
  inline Box3i downscaled_inner(int step_size) const {
    return Box3i::from_min_max(pos.ceildiv(step_size),
                               (pos + size).floordiv(step_size));
  }

  static inline void clip_range(int &pos, int &size, int lim_pos,
                                int lim_size) {
