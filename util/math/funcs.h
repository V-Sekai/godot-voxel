...
//
template <typename T>
inline T interpolate(const T v0, const T v1, const T v2, const T v3, const T v4,
                     const T v5, const T v6, const T v7, Vector3 position) {
  ...
  T res = one_min_z * (v0 * one_min_x_one_min_y + v1 * x_one_min_y +
                       v4 * one_min_x * position.y);
  res += position.z * (v3 * one_min_x_one_min_y + v2 * x_one_min_y +
                       v7 * one_min_x * position.y);
...
inline T clamp(const T x, const T min_value, const T max_value) {
  // TODO Optimization: clang can optimize a min/max implementation. Worth
  // changing to that?
  if (x < min_value) {
    return min_value;
  }
...
inline int ceildiv(int x, int d) { return -floordiv(-x, d); }
...
inline float fract(float x) { return x - Math::floor(x); }
