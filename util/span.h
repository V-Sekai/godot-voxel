...
public:
  ...
  // TODO Remove this one, prefer to_span() specializations
  template <unsigned int N> inline Span(FixedArray<T, N> &a) {
    _ptr = a.data();
    _size = a.size();
  }
  ...
  template <typename U> Span<U> reinterpret_cast_to() const {
...
template <typename T> Span<T> to_span(std::vector<T> &vec) {
...
template <typename T> Span<const T> to_span_const(const std::vector<T> &vec) {
  return Span<const T>(vec.data(), 0, vec.size());
}

template <typename T, unsigned int N> Span<T> to_span(FixedArray<T, N> &a) {
