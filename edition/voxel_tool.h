...
public:
	...
	virtual void do_circle(VoxelVector3i pos, int radius, VoxelVector3i direction);
	...
	virtual void copy(VoxelVector3i pos, Ref<VoxelBuffer> dst, uint8_t channels_mask);
	virtual void paste(VoxelVector3i pos, Ref<VoxelBuffer> p_voxels, uint8_t channels_mask, uint64_t mask_value);
...
private:
	...
	void _b_do_point(Vector3 pos) {
		do_point(VoxelVector3i::from_floored(pos));
	}
	void _b_do_line(Vector3 begin, Vector3 end) {
		do_line(VoxelVector3i::from_floored(begin), VoxelVector3i::from_floored(end));
	}
	void _b_do_circle(Vector3 pos, float radius, Vector3 direction) {
		do_circle(VoxelVector3i::from_floored(pos), radius, VoxelVector3i::from_floored(direction));
	}
	...
	void _b_paste(Vector3 pos, Ref<RefCounted> voxels, int channels_mask, int mask_value) {
	...
	bool _b_is_area_editable(AABB box) {
		return is_area_editable(Box3i(VoxelVector3i::from_floored(box.position), VoxelVector3i::from_floored(box.size)));
	}
...
protected:
	...
	struct TextureBlendSphereOp {
		...
		inline void operator()(VoxelVector3i pos, uint16_t &indices, uint16_t &weights) const {
