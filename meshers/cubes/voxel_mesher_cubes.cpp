...
const uint8_t g_face_axes_lut[VoxelVector3i::AXIS_COUNT][2] = {
    // X
    {VoxelVector3i::AXIS_Y, VoxelVector3i::AXIS_Z},
    // Y
    {VoxelVector3i::AXIS_X, VoxelVector3i::AXIS_Z},
    // Z
    {VoxelVector3i::AXIS_X, VoxelVector3i::AXIS_Y}};
...
// 2 if alpha is max
inline uint8_t get_alpha_index(Color8 c) { return (c.a == 0xf) + (c.a > 0); }

template <typename Voxel_T, typename Color_F>
void build_voxel_mesh_as_simple_cubes(
    FixedArray<VoxelMesherCubes::Arrays, VoxelMesherCubes::MATERIAL_COUNT>
  ...
  const VoxelVector3i min_pos = VoxelVector3i(VoxelMesherCubes::PADDING);
  const VoxelVector3i max_pos =
      block_size - VoxelVector3i(VoxelMesherCubes::PADDING);
  ...
  FixedArray<uint32_t, VoxelVector3i::AXIS_COUNT> neighbor_offset_d_lut;
  neighbor_offset_d_lut[VoxelVector3i::AXIS_X] = block_size.y;
  neighbor_offset_d_lut[VoxelVector3i::AXIS_Y] = 1;
  neighbor_offset_d_lut[VoxelVector3i::AXIS_Z] = block_size.x * block_size.y;

  FixedArray<uint32_t, VoxelMesherCubes::MATERIAL_COUNT> index_offsets(0);

  // For each axis
  for (unsigned int za = 0; za < VoxelVector3i::AXIS_COUNT; ++za) {
    const unsigned int xa = g_face_axes_lut[za][0];
    const unsigned int ya = g_face_axes_lut[za][1];

    // For each deck
    for (unsigned int d = min_pos[za] - VoxelMesherCubes::PADDING;
         d < (unsigned int)max_pos[za]; ++d) {
      // For each cell of the deck, gather face info
      for (unsigned int fy = min_pos[ya]; fy < (unsigned int)max_pos[ya];
           ++fy) {
        for (unsigned int fx = min_pos[xa]; fx < (unsigned int)max_pos[xa];
             ++fx) {
          FixedArray<unsigned int, VoxelVector3i::AXIS_COUNT> pos;
          pos[xa] = fx;
          pos[ya] = fy;
          pos[za] = d;

          const unsigned int voxel_index =
              pos[VoxelVector3i::AXIS_Y] +
              pos[VoxelVector3i::AXIS_X] * row_size +
              pos[VoxelVector3i::AXIS_Z] * deck_size;

          const Voxel_T raw_color0 = voxel_buffer[voxel_index];
          const Voxel_T raw_color1 =
              voxel_buffer[voxel_index + neighbor_offset_d_lut[za]];
          ...
          VoxelMesherCubes::Arrays &arrays =
              out_arrays_per_material[material_index];
...
void build_voxel_mesh_as_greedy_cubes(
    FixedArray<VoxelMesherCubes::Arrays, VoxelMesherCubes::MATERIAL_COUNT>
        &out_arrays_per_material,
    const Span<Voxel_T> voxel_buffer, const VoxelVector3i block_size,
    std::vector<uint8_t> &mask_memory_pool, Color_F color_func) {
  //
  ERR_FAIL_COND(
      block_size.x < static_cast<int>(2 * VoxelMesherCubes::PADDING) ||
      block_size.y < static_cast<int>(2 * VoxelMesherCubes::PADDING) ||
  ...
  for (unsigned int za = 0; za < VoxelVector3i::AXIS_COUNT; ++za) {
    ...
    Span<MaskValue> mask(reinterpret_cast<MaskValue *>(mask_memory_pool.data()),
                         0, mask_area);
    ...
    for (unsigned int d = min_pos[za] - VoxelMesherCubes::PADDING;
      ...
      for (unsigned int fy = min_pos[ya]; fy < (unsigned int)max_pos[ya];
        ...
        for (unsigned int fx = min_pos[xa]; fx < (unsigned int)max_pos[xa];
          ...
          mask[(fx - VoxelMesherCubes::PADDING) +
               (fy - VoxelMesherCubes::PADDING) * mask_size_x] = mv;
        }
      }

      struct L {
        static inline bool is_range_equal(const Span<MaskValue> &mask,
      ...
      for (unsigned int fy = 0; fy < mask_size_y; ++fy) {
        for (unsigned int fx = 0; fx < mask_size_x; ++fx) {
          ...
          while (ry < mask_size_y &&
                 L::is_range_equal(mask, fx + ry * mask_size_x,
                                   rx + ry * mask_size_x, m)) {
            ++ry;
          }
...
void build_voxel_mesh_as_greedy_cubes_atlased(
  ...
  for (unsigned int za = 0; za < VoxelVector3i::AXIS_COUNT; ++za) {
    ...
    Span<MaskValue> mask(reinterpret_cast<MaskValue *>(mask_memory_pool.data()),
                         0, mask_area);
    Span<Color8> colors(
        reinterpret_cast<Color8 *>(mask_memory_pool.data() + mask_memory_size),
        0, mask_area);
    ...
    for (unsigned int d = min_pos[za] - VoxelMesherCubes::PADDING;
      ...
      for (unsigned int fy = min_pos[ya]; fy < (unsigned int)max_pos[ya];
        ...
        for (unsigned int fx = min_pos[xa]; fx < (unsigned int)max_pos[xa];
          ...
          const unsigned int mask_index =
              (fx - VoxelMesherCubes::PADDING) +
              (fy - VoxelMesherCubes::PADDING) * mask_size_x;
          mask[mask_index] = mv;
      ...
      for (unsigned int fy = 0; fy < mask_size_y; ++fy) {
        for (unsigned int fx = 0; fx < mask_size_x; ++fx) {
          ...
          arrays.uvs.resize(arrays.uvs.size() +
                            4); // Values will be assigned in a second pass
          ...
          out_greedy_atlas_data.colors.resize(
              out_greedy_atlas_data.colors.size() +
          ...
          for (unsigned int my = fy; my < ry; ++my) {
            ...
            // TODO Actually that code only missed an offset to its destination
            // for each row? Copy colors row by row
            // memcpy(out_greedy_atlas_data.colors.data() +
            // image_info.first_color_index, 		colors.data() + i0,
            // 		image_info.size_x * sizeof(Color8));
          }

          // TODO Optimization: if colors are uniform, we could allocate a
          // shared single pixel instead. This would reduce texture size and
          // packing cost
...
static Ref<Image>
make_greedy_atlas(const VoxelMesherCubes::GreedyAtlasData &atlas_data,
  ...
  Vector2i result_size;
  {
    ...
    for (unsigned int i = 0; i < atlas_data.images.size(); ++i) {
      const VoxelMesherCubes::GreedyAtlasData::ImageInfo &im =
          atlas_data.images[i];
  ...
  // 	const VoxelMesherCubes::GreedyAtlasData::ImageInfo &im =
  // atlas_data.images[i]; 	Ref<Image> tmp; 	tmp.instantiate();
  // 	tmp->create(im.size_x, im.size_y, false, debug_im->get_format());
  // 	tmp->fill(Color(Math::randf(), Math::randf(), Math::randf()));
  // 	debug_im->blit_rect(tmp, Rect2(0, 0, tmp->get_width(),
  // tmp->get_height()), Vector2(dst_pos));
  // }
  // debug_im->save_png("debug_atlas_packing.png");

  // Update UVs
  const Vector2 uv_scale(1.f / float(result_size.x),
                         1.f / float(result_size.y));
  for (unsigned int i = 0; i < atlas_data.images.size(); ++i) {
    const VoxelMesherCubes::GreedyAtlasData::ImageInfo &im =
        atlas_data.images[i];
    VoxelMesherCubes::Arrays &surface = surfaces[im.surface_index];
    ERR_FAIL_COND_V(im.first_vertex_index + 4 > surface.uvs.size(),
                    Ref<Image>());
  ...
  {
    Span<Color8> dst_data =
        Span<Color8>(reinterpret_cast<Color8 *>(im_data.ptrw()),
    ...
    for (unsigned int i = 0; i < atlas_data.images.size(); ++i) {
      const VoxelMesherCubes::GreedyAtlasData::ImageInfo &im =
          atlas_data.images[i];
      const Vector2i dst_pos = result_points[i];
      Span<const Color8> src_data = const_span_from_position_and_size(
          atlas_data.colors, im.first_color_index, im.size_x * im.size_y);

      // Blit rectangle
      for (unsigned int y = 0; y < im.size_y; ++y) {
        for (unsigned int x = 0; x < im.size_x; ++x) {
          const unsigned int src_i = x + y * im.size_x;
          const unsigned int dst_i =
              (dst_pos.x + x) + (dst_pos.y + y) * result_size.x;
...
VoxelMesherCubes::VoxelMesherCubes() { set_padding(PADDING, PADDING); }

VoxelMesherCubes::~VoxelMesherCubes() {}

void VoxelMesherCubes::build(VoxelMesher::Output &output,
  ...
  // This is the most intensive job in this class, so all required data should
  // be as fit as possible.

  // The buffer we receive MUST be dense (i.e not compressed, and channels
  // allocated). That means we can use raw pointers to voxel data inside instead
  // of using the higher-level getters, and then save a lot of time.

  if (voxels.get_channel_compression(channel) ==
  ...
  } else if (voxels.get_channel_compression(channel) !=
             VoxelBuffer::COMPRESSION_NONE) {
  ...
  case COLOR_RAW:
    ...
    case VoxelBuffer::DEPTH_8_BIT:
      if (params.greedy_meshing) {
        build_voxel_mesh_as_greedy_cubes(cache.arrays_per_material, raw_channel,
                                         block_size, cache.mask_memory_pool,
                                         Color8::from_u8);
      } else {
        build_voxel_mesh_as_simple_cubes(cache.arrays_per_material, raw_channel,
                                         block_size, Color8::from_u8);
      }
      break;
    ...
    case VoxelBuffer::DEPTH_16_BIT:
      if (params.greedy_meshing) {
        build_voxel_mesh_as_greedy_cubes(
            ...
            raw_channel.reinterpret_cast_to<uint16_t>(), block_size,
            cache.mask_memory_pool, Color8::from_u16);
      } else {
  ...
  case COLOR_MESHER_PALETTE: {
    ERR_FAIL_COND_MSG(params.palette.is_null(),
                      "Palette mode is used but no palette was specified");

    struct GetColorFromPalette {
      VoxelColorPalette &palette;
      Color8 operator()(uint64_t i) const {
        // Note: even though this code may run in a thread, I'm not locking the
        // palette at all because it stores colors in a fixed-size array, and
        // reading the wrong color won't cause any serious problem. It's not
        // supposed to change often in game anyways. If it does, better use
        // shader mode.
      ...
      if (params.greedy_meshing) {
        if (params.store_colors_in_texture) {
          build_voxel_mesh_as_greedy_cubes_atlased(
              cache.arrays_per_material, cache.greedy_atlas_data, raw_channel,
              block_size, cache.mask_memory_pool, get_color_from_palette);
          atlas_image = make_greedy_atlas(cache.greedy_atlas_data,
                                          to_span(cache.arrays_per_material));
        } else {
          build_voxel_mesh_as_greedy_cubes(
              cache.arrays_per_material, raw_channel, block_size,
              cache.mask_memory_pool, get_color_from_palette);
        }
      } else {
        build_voxel_mesh_as_simple_cubes(cache.arrays_per_material, raw_channel,
                                         block_size, get_color_from_palette);
      }
      break;
  ...
  case COLOR_SHADER_PALETTE: {
    ...
    struct GetIndexFromPalette {
      VoxelColorPalette &palette;
      Color8 operator()(uint64_t i) const {
        // Still providing alpha because it allows to separate the opaque and
    ...
    case VoxelBuffer::DEPTH_8_BIT:
      if (params.greedy_meshing) {
        ...
        build_voxel_mesh_as_simple_cubes(cache.arrays_per_material, raw_channel,
                                         block_size, get_index_from_palette);
      }
      break;
...
void VoxelMesherCubes::_bind_methods() {
  ...
  ClassDB::bind_method(D_METHOD("is_greedy_meshing_enabled"),
                       &VoxelMesherCubes::is_greedy_meshing_enabled);

  ClassDB::bind_method(D_METHOD("set_palette", "palette"),
                       &VoxelMesherCubes::set_palette);
  ClassDB::bind_method(D_METHOD("get_palette"), &VoxelMesherCubes::get_palette);

  ClassDB::bind_method(D_METHOD("set_color_mode", "mode"),
                       &VoxelMesherCubes::set_color_mode);
  ClassDB::bind_method(D_METHOD("get_color_mode"),
                       &VoxelMesherCubes::get_color_mode);

  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "greedy_meshing_enabled"),
               "set_greedy_meshing_enabled", "is_greedy_meshing_enabled");
  ADD_PROPERTY(PropertyInfo(Variant::INT, "color_mode"), "set_color_mode",
               "get_color_mode");
  ADD_PROPERTY(PropertyInfo(Variant::OBJECT, "palette",
                            PROPERTY_HINT_RESOURCE_TYPE, "VoxelColorPalette"),
