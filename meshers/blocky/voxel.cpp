...
Voxel::Voxel()
    : _id(-1), _material_id(0), _transparency_index(0), _color(1.f, 1.f, 1.f),
...
void Voxel::set_voxel_name(String name) { _name = name; }

void Voxel::set_id(int id) {
  ERR_FAIL_COND(id < 0 || (unsigned int)id >= VoxelLibrary::MAX_VOXEL_TYPES);
  // Cannot modify ID after creation
  ERR_FAIL_COND_MSG(_id != -1,
                    "ID cannot be modified after being added to a library");
  _id = id;
}

void Voxel::set_color(Color color) { _color = color; }
...
Voxel::GeometryType Voxel::get_geometry_type() const { return _geometry_type; }

void Voxel::set_custom_mesh(Ref<Mesh> mesh) { _custom_mesh = mesh; }

void Voxel::set_cube_geometry() {}

void Voxel::set_random_tickable(bool rt) { _random_tickable = rt; }
...
void Voxel::set_collision_mask(uint32_t mask) { _collision_mask = mask; }

static void bake_cube_geometry(Voxel &config, Voxel::BakedData &baked_data,
                               int p_atlas_size, bool bake_tangents) {
  const float sy = 1.0;
  ...
  // I am confused. I read in at least 3 OpenGL tutorials that texture
  // coordinates start at bottom-left (0,0). But even though Godot is said to
  // follow OpenGL's convention, the engine starts at top-left!
  const Vector2 uv[4] = {
      Vector2(e, 1.f - e),
      Vector2(1.f - e, 1.f - e),
      Vector2(1.f - e, e),
...
static void bake_mesh_geometry(Voxel &config, Voxel::BakedData &baked_data,
  ...
  ERR_FAIL_COND_MSG(indices.size() % 3 != 0,
                    "Mesh is empty or does not contain triangles");
  ...
  struct L {
    static uint8_t get_sides(Vector3 pos) {
      ...
      mask |= Math::is_equal_approx(pos.x, real_t(0.0), tolerance)
              << Cube::SIDE_NEGATIVE_X;
      mask |= Math::is_equal_approx(pos.x, real_t(1.0), tolerance)
              << Cube::SIDE_POSITIVE_X;
      mask |= Math::is_equal_approx(pos.y, real_t(0.0), tolerance)
              << Cube::SIDE_NEGATIVE_Y;
      mask |= Math::is_equal_approx(pos.y, real_t(1.0), tolerance)
              << Cube::SIDE_POSITIVE_Y;
      mask |= Math::is_equal_approx(pos.z, real_t(0.0), tolerance)
              << Cube::SIDE_NEGATIVE_Z;
      mask |= Math::is_equal_approx(pos.z, real_t(1.0), tolerance)
              << Cube::SIDE_POSITIVE_Z;
      return mask;
    }

    static bool get_triangle_side(const Vector3 &a, const Vector3 &b,
                                  const Vector3 &c, Cube::SideAxis &out_side) {
  ...
  if (tangents_empty && bake_tangents) {
    WARN_PRINT(String("Voxel model '{0}' with ID {1} does not have tangents. "
                      "They will be generated."
                      "You should consider providing a mesh with tangents, or "
                      "at least UVs and normals, "
  ...
  // Separate triangles belonging to faces of the cube
  {
    ...
    for (int i = 0; i < indices.size(); i += 3) {
      ...
      if (tangents_empty && bake_tangents) {
        ...
        Vector3 bt =
            (delta_pos2 * delta_uv1[0] - delta_pos1 * delta_uv2[0]) * r;
        tangent[0] = t[0];
        tangent[1] = t[1];
        tangent[2] = t[2];
      ...
      if (L::get_triangle_side(tri_positions[0], tri_positions[1],
                               tri_positions[2], side)) {
        ...
        for (int j = 0; j < 3; ++j) {
          ...
          const int *existing_dst_index =
              added_side_indices[side].getptr(src_index);
          ...
          if (existing_dst_index == nullptr) {
            ...
            if (bake_tangents) {
              if (tangents_empty) {
                ...
                // i is the first vertex of each triangle which increments by
                // steps of 3. There are 4 floats per tangent.
                int ti = (i / 3) * 4;
        ...
        for (int j = 0; j < 3; ++j) {
          int src_index = indices[i + j];
          const int *existing_dst_index =
              added_regular_indices.getptr(src_index);
...
void Voxel::_bind_methods() {
  ClassDB::bind_method(D_METHOD("set_voxel_name", "name"),
                       &Voxel::set_voxel_name);
  ...
  ClassDB::bind_method(D_METHOD("set_transparent", "transparent"),
                       &Voxel::set_transparent);
  ClassDB::bind_method(D_METHOD("is_transparent"), &Voxel::is_transparent);

  ClassDB::bind_method(D_METHOD("set_transparency_index", "transparency_index"),
                       &Voxel::set_transparency_index);
  ClassDB::bind_method(D_METHOD("get_transparency_index"),
                       &Voxel::get_transparency_index);

  ClassDB::bind_method(D_METHOD("set_random_tickable", "rt"),
                       &Voxel::set_random_tickable);
  ClassDB::bind_method(D_METHOD("is_random_tickable"),
                       &Voxel::is_random_tickable);

  ClassDB::bind_method(D_METHOD("set_material_id", "id"),
                       &Voxel::set_material_id);
  ClassDB::bind_method(D_METHOD("get_material_id"), &Voxel::get_material_id);

  ClassDB::bind_method(D_METHOD("set_geometry_type", "type"),
                       &Voxel::set_geometry_type);
  ClassDB::bind_method(D_METHOD("get_geometry_type"),
                       &Voxel::get_geometry_type);

  ClassDB::bind_method(D_METHOD("set_custom_mesh", "type"),
                       &Voxel::set_custom_mesh);
  ClassDB::bind_method(D_METHOD("get_custom_mesh"), &Voxel::get_custom_mesh);

  ClassDB::bind_method(D_METHOD("set_collision_aabbs", "aabbs"),
                       &Voxel::_b_set_collision_aabbs);
  ClassDB::bind_method(D_METHOD("get_collision_aabbs"),
                       &Voxel::_b_get_collision_aabbs);

  ClassDB::bind_method(D_METHOD("set_collision_mask", "mask"),
                       &Voxel::set_collision_mask);
  ClassDB::bind_method(D_METHOD("get_collision_mask"),
                       &Voxel::get_collision_mask);

  ClassDB::bind_method(D_METHOD("is_empty"), &Voxel::is_empty);

  // TODO Update to StringName in Godot 4
  ADD_PROPERTY(PropertyInfo(Variant::STRING, "voxel_name"), "set_voxel_name",
               "get_voxel_name");
  ADD_PROPERTY(PropertyInfo(Variant::COLOR, "color"), "set_color", "get_color");
  // TODO Might become obsolete
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "transparent", PROPERTY_HINT_NONE,
                            "", PROPERTY_USAGE_STORAGE),
               "set_transparent", "is_transparent");
  ADD_PROPERTY(PropertyInfo(Variant::INT, "transparency_index"),
               "set_transparency_index", "get_transparency_index");
  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "random_tickable"),
               "set_random_tickable", "is_random_tickable");
  ADD_PROPERTY(PropertyInfo(Variant::INT, "material_id"), "set_material_id",
               "get_material_id");
  ADD_PROPERTY(PropertyInfo(Variant::INT, "geometry_type", PROPERTY_HINT_ENUM,
                            "None,Cube,CustomMesh"),
               "set_geometry_type", "get_geometry_type");
  ADD_PROPERTY(PropertyInfo(Variant::OBJECT, "custom_mesh",
                            PROPERTY_HINT_RESOURCE_TYPE, "Mesh"),
               "set_custom_mesh", "get_custom_mesh");
  ADD_PROPERTY(PropertyInfo(Variant::ARRAY, "collision_aabbs",
