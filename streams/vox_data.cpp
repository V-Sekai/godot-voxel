...
//   MagicaVoxel     OpenGL
//
inline VoxelVector3i magica_to_opengl(const VoxelVector3i &src) {
  VoxelVector3i dst;
  dst.x = src.y;
  dst.y = src.z;
  dst.z = src.x;
...
void transpose(VoxelVector3i sx, VoxelVector3i sy, VoxelVector3i sz,
               VoxelVector3i &dx, VoxelVector3i &dy, VoxelVector3i &dz) {
  dx.x = sx.x;
  dx.y = sy.x;
  dx.z = sz.x;
...
static Basis parse_basis(uint8_t data) {
  ...
  // The index of the non-zero entry in the last row can be deduced as the last
  // not "occupied" index
  ...
  // The following is a bit messy, had a hard time figuring out the correct
  // combination of conversions to bring MagicaVoxel rotations to Godot
  // rotations.
...
Error Data::_load_from_file(String fpath) {
  ...
  while (f.get_position() < file_length) {
    ...
    if (strcmp(chunk_id, "SIZE") == 0) {
      ...
      if (t_it != frame.end()) {
        ...
        // coords[1], coords[2])));
        node.position =
            magica_to_opengl(VoxelVector3i(coords[0], coords[1], coords[2]));
      }
      ...
      for (unsigned int i = 0; i < _layers.size(); ++i) {
        ...
        ERR_FAIL_COND_V_MSG(existing_layer->id == layer_id, ERR_INVALID_DATA,
                            String("Layer with ID {0} already exists")
                                .format(varray(layer_id)));
      }
      layer.id = layer_id;
      ...
      ERR_FAIL_COND_V(material_id < 0 ||
                      ...
                      ERR_INVALID_DATA);
      ERR_FAIL_COND_V_MSG(
          _materials.find(material_id) != _materials.end(), ERR_INVALID_DATA,
  ...
  // There is no indication on the official spec to detect the root node of the
  // scene graph. It might just be the first one we find in the file, but the
  // specification does not explicitely enforce that. So we have to do it the
  // long way, marking which nodes are referenced by others.
  ...
  for (auto it = _scene_graph.begin(); it != _scene_graph.end(); ++it) {
    ...
    case Node::TYPE_TRANSFORM: {
      const TransformNode *transform_node =
          reinterpret_cast<const TransformNode *>(node);

      const int child_id = transform_node->child_node_id;
      ERR_FAIL_COND_V_MSG(
          _scene_graph.find(child_id) == _scene_graph.end(), ERR_INVALID_DATA,
      ...
      // Apparently it is possible to find nodes that are not attached to any
      // layer
      if (layer_id != -1) {
  ...
  for (auto it = _scene_graph.begin(); it != _scene_graph.end(); ++it) {
    ...
    ERR_FAIL_COND_V_MSG(_root_node_id != -1, ERR_INVALID_DATA,
...
unsigned int Data::get_model_count() const { return _models.size(); }
...
int Data::get_root_node_id() const { return _root_node_id; }

unsigned int Data::get_layer_count() const { return _layers.size(); }
